...
$usermedecin = $_GET['log'];
$cle = $_GET['cle'];
 
// Récupération de la clé correspondant au $login dans la base de données
$stmt = $rdvsoins->prepare("SELECT cle,actif FROM medecin WHERE usermedecin like :usermedecin ");
if($stmt->execute(array(':usermedecin' => $usermedecin)) && $row = $stmt->fetch())
  {
     ...
     if($cle == $clebdd) // On compare nos deux clés    
       {
          ...
          $stmt = $rdvsoins->prepare("UPDATE medecin SET actif = 1 WHERE usermedecin like :usermedecin ");
